...
### Fixed
- **Order book memory leak (unbounded → capped)** — the shared `OrderBook` (Binance/Bybit) only removes a price level on an explicit 0-qty diff, which exchanges send solely for levels inside their active window. A level that drifts far from the mid is never removed, so a long-lived book — Binance perp resyncs ~once per 6 days — accumulated stale deep levels without bound (observed ~800 MB anon RSS over 6 days, approaching the 1 GiB container limit). `bids`/`asks` and their `bid_last`/`ask_last` mirrors now cap at `BOOK_LEVEL_CAP` (1000) levels per side, pruning the farthest-from-mid levels past a `BOOK_PRUNE_TRIGGER` (1200) hysteresis threshold. We only ever read the top 10 levels (snapshot columns, OFI, microprice) and full depth is preserved in the raw-diff stream, so pruning is lossless. Same root cause as the 0.3.0 dYdX crossed-book fix, different symptom.
