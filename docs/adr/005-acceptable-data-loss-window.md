...
## Errata (2026-07-04): restart truncation was the real worst case for 1s/deriv

The "≤5 minutes" (1s snapshot) and "zero loss on graceful shutdown" figures above describe steady-state buffering loss. They did not account for a separate, worse failure mode: `snap_1s.rs` and `deriv.rs` wrote one file per calendar day, opened lazily via a `HashMap` that started empty every process restart. A mid-day restart's `File::create` truncated the existing day's file — there was no distinction between "first event of a new day" and "process restarted mid-day". Confirmed in production 2026-07-04: 3 fathom restarts destroyed ~22h of one day's data and ~3.5h of another day's data in the 1s/deriv feeds. The actual pre-fix worst case for these two writers was **unbounded, up to ~24h**, not the 5s/5min figures above (those never applied to the restart scenario, only to a clean in-memory-buffer crash within an already-open file).

**Fix:** `snap_1s.rs` and `deriv.rs` now use the same hourly `Bucket` open/rename lifecycle `raw.rs`/`trades.rs` already had (`src/writer/rotation.rs`) — a uniquely-named temp file per bucket, renamed into place only on rotation or graceful shutdown. This bounds restart loss to **at most one open bucket, sized to the configured `raw_rotate_hours`** (currently 1h in prod/default) — not a hardcoded "≤1h" claim, since the config allows any divisor of 24 up to 24h.

This is a bounded-loss design, not full restart-safety: the bucket open at crash time is still entirely lost. An `ArrowWriter` that never reaches `.finish()` has no Parquet footer, so the open bucket's file is not "missing a few rows" — it is not a valid Parquet file at all. Deriv additionally gained a periodic force-rotate (via an injected `Clock`) so a sparse feed (e.g. `liq`) can't hold an `_open.parquet` file open indefinitely past its bucket boundary, which would otherwise leave an orphaned, unrecoverable file on a crash.
