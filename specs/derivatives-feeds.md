...
- Parquet: `{data_dir}/deriv/{exchange}/{symbol}/{date}/{feed}.parquet`, `feed` ∈ `funding|oi|liq` — **daily** files (rates are low; hourly rotation unnecessary). Writer: `src/writer/deriv.rs::run_deriv_writer` — daily file keyed by event-time date (snap_1s pattern) + 5 s buffer flush with forced row groups (raw pattern); the composition trade-off is documented in the module doc. Arrow schemas: `src/schema.rs::{mark_funding_schema, open_interest_schema, liquidation_schema}`.
