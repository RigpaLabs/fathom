...
pub struct BybitPerp;

impl ExchangeAdapter for BybitPerp {
    fn name(&self) -> &str {
        "bybit_perp"
    }
    ...
    // open_interest_url: not overridden — Bybit's `tickers` WS channel
    // already carries openInterest/openInterestValue, so the trait's
    // default `None` is correct here (no REST OI poll for Bybit at all,
    // simpler than Binance perp; see specs/bybit-collection.md).
}
